...
function ctrl($rootScope, $http) {
  ...
  function deleteItem(item) {
    $http.put(`${baseUri}/items/${item._id}`, {
      completed: true
    }).then(() => {
      getList();
    });
  ...
  function saveEdit(index) {
    if (vm.editedNumber) {
      ...
      $http.put(`${baseUri}/items/${vm.list[index]._id}`, {
...
function refreshButton() {
  return {
    restrict: 'E',
    template: '<button name="ref" class="btn btn-info" ng-click="refreshItem()">refresh</button>',
    link: function(scope, element, attrs) {
      ...
      function refreshItem() {
        ...
        var url;
        if (!!~number.toLowerCase().indexOf('at') ||
          !!~number.toLowerCase().indexOf('td') ||
          !!~number.toLowerCase().indexOf('al')) {
          //aol
          url = 'http://aol-au.com/admin/weixin/waybill/toQueryResult?code=' + number;
        ...
        } else if (number.toLowerCase().indexOf('aj') === 0 || number.toLowerCase().indexOf('em') === 0) {
          //AJ
          ...
          var parent = element.parent().parent();
          submitForm(url, parent, 'cno', number);
        } else if (number.indexOf('2000') === 0) {
          url = 'http://www.auodexpress.com/website/doAuodList.do';
          var parent = element.parent().parent();
          submitForm(url, parent, 'waybillNum', number);
        } else if (number.toLowerCase().indexOf('cj') === 0 || number.toLowerCase().indexOf('ce') === 0) {
...
function closeButton() {
  return {
    restrict: 'E',
    template: '<button type="button" name="close"  class="btn btn-warning" ng-click="closeIframe()">hide</button>',
    link: function(scope, element, attrs) {

      scope.closeIframe = closeIframe;

      function closeIframe() {
        var iframe = element.parent().parent().find('iframe');
        if (iframe.length > 0) {
